...
function ENT:Think()
	...
	for k,v in pairs(ents) do
		...
		if IsValid(self:GetOwner()) and IsValid(tr.Entity) and tr.Entity:IsNPC() and tr.Entity != self:GetOwner() then
			local dmginfo = DamageInfo()
...
local function OwnerDamage(vict, dmginfo)
	local attacker = dmginfo:GetAttacker()
	local inflictor = dmginfo:GetInflictor()
	if IsValid(inflictor) and IsValid(attacker) and inflictor:GetClass() == "doom3_bfg" and vict == attacker then
		dmginfo:ScaleDamage(0)
	end
end
hook.Add("EntityTakeDamage", "BFGDamage", OwnerDamage)